...
mpgc = mpc2gas_prep_modif(mpgc,mpopt);
%% run program
...
tol = 1e-3;
if any(dem_diff > tol) 
    error('mpng: Total gas demand does not correspond to the sum of the diversified demand.');
end
...
ngtemp = size(mpc.gen,1) - 0; %size(mpc.gen,1) - ndl;  %modif  
if ~isempty(connect.interc.term)    
    ng_ori = ngtemp/nt;
    gens = (1:ngtemp)';
    gens = reshape(gens,ng_ori,nt);
    id_pggas = connect.interc.term(:,GEN_ID);   %modif
...
%  ------------------------------------------------------------------------
%% Power demanded by compressors must be equal for all periods of time and
...
% Gas-fired units
if ~isempty(connect.interc.term)
    id_gfu = connect.interc.term(:,1);  %modif
    node_gfu = connect.interc.term(:,2);
    eff_gfu = connect.interc.term(:,3);
%     pg=results.gen(1:86,2); %modif Ngen=86 
pg = results.var.val.Pg*wbase;
    ndl = 0; %results.nsd.N; %modif
    pg_gfu_day = pg(1:end-ndl);   %modif
...
%   code to save the results of the optimal power and natural gas flow.
%
%   Not currently used.
%
end
